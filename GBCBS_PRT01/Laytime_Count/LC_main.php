...
<!--CONFIRM関連-->
    <div class="shipment_confirmation">
        <a class="shipment_confirmation_button" href="./confirmation_input.php">確認配船登録</a>    
    </div>

    <div class="shipment_table01">
        <form method="post" action="">
            <table class="confirmation_table02">
                <tr>
                    <th>船名</th>
                    <th>案件名</th>
                    <th>契約年度</th>
                ...
                <?php
                $pdo = db_connect();
                $stmt = $pdo->prepare("SELECT * FROM shipments WHERE confirm_id = :user_id");
                $stmt->bindValue(':user_id', $user_id, PDO::PARAM_INT);
